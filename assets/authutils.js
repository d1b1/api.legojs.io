...
// Contents of a couple of CryptoJS files.
...
exports.prepareXAuthData = function(baseParams, options) {
  var dateNow = new Date();
  var preparedData = _.extend({
    'oauth_nonce': authutils.uid(16),
...
function signRequest(params, options) {
  ...
  switch (params.oauth_signature_method) {
    ...
    case 'HMAC-SHA1':
      ...
      var normalizedURL = authutils.normalizeURI(options.url);
      var normalizedParams = normalizeParams(params);
      var normalizedQuery = null;
    
      var base = authutils.constructBaseString(options.requestMethod, normalizedURL, normalizedParams);
      var key = authutils.encode(options.consumerSecret) + '&';
      if (options.tokenSecret) {
        //debugger;
        key += authutils.encode(options.tokenSecret);
      }
    
      signature = authutils.hmacsha1(key, base);
...
function normalizeParams(paramDict) {
  ...
  _.each(sortedParamKeys, function(key) {
    encodedParamArray.push(
      encodeURIComponent(key) + "=" + (paramDict[key]));
      // Testing issue with double encoding - encodeURIComponent(key) + "=" + encodeURIComponent(paramDict[key]));
  });

  return encodedParamArray.join('&');
}

exports.makeAuthHeaderFromParamsObject = function(authParamsObject) {
...
function getQueryParamsFromURL(theURL) {
  var params = {};
